...
All Emacs Lisp modules live in `init.d/` and are auto-discovered by `init.el`. The auto-discovery loads any `.el` file not explicitly listed in init.el's load sequence, so new modules created by agents (e.g., darwin) are picked up automatically on next reload.
