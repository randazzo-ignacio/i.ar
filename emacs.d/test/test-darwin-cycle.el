...
;;; Tests for agent_cycle.el (formerly darwin_cycle.el)
;; Tests the pure helper functions: agent--cycle-complete-p and
;; agent--load-profile. The main agent-run-cycle function involves
...
;;; --- agent--cycle-complete-p tests ---

(ert-deftest test-darwin-cycle-complete-with-markers ()
  "agent--cycle-complete-p should return t when buffer has completion + history."
  (with-temp-buffer
...
(ert-deftest test-darwin-cycle-complete-no-completion-marker ()
  ...
  (with-temp-buffer
    ...
    (should (null (agent--cycle-complete-p (current-buffer))))))

(ert-deftest test-darwin-cycle-complete-no-history-reference ()
  "agent--cycle-complete-p should return nil without history reference."
  (with-temp-buffer
    (insert "Cycle complete. All steps done.\n")
...
(ert-deftest test-darwin-cycle-complete-empty-buffer ()
  "agent--cycle-complete-p should return nil for empty buffer."
  (with-temp-buffer
    (should (null (agent--cycle-complete-p (current-buffer))))))
...
(ert-deftest test-darwin-cycle-complete-region-only ()
  "agent--cycle-complete-p should find completion markers within START/END region.
...
function should return nil -- fixing the false positive bug."
  (with-temp-buffer
    ...
    (let* ((sep-end (save-excursion
      ...
      (should (null (agent--cycle-complete-p (current-buffer) region-start region-end))))))

(ert-deftest test-darwin-cycle-complete-region-no-history-in-region ()
  "agent--cycle-complete-p should return nil when HISTORY is outside region.
...
(ert-deftest test-darwin-cycle-complete-region-nil-args-searches-all ()
  "agent--cycle-complete-p should search entire buffer when START/END are nil.
...
(ert-deftest test-darwin-load-profile-returns-string ()
  "agent--load-profile should return a string (darwin's profile)."
  (let ((profile (agent--load-profile "darwin")))
    (should (stringp profile))
    (should (> (length profile) 0))))

(ert-deftest test-darwin-load-profile-contains-darwin-identity ()
  "agent--load-profile should contain Darwin's identity text."
  (let ((profile (agent--load-profile "darwin")))
    (should (string-match-p "Darwin" profile))))

(ert-deftest test-darwin-load-profile-errors-on-missing ()
  "agent--load-profile should error when prompt.org is missing.
...
signaled with the expected message."
  (let ((user-emacs-directory (make-temp-file "test-darwin-" :dir-flag)))
    (unwind-protect
        (let ((err (should-error (agent--load-profile "nonexistent") :type 'error)))
          (should (string-match-p "not found" (cadr err))))
      (delete-directory user-emacs-directory t))))

;;; --- agent--notify-telegram tests ---
...
(ert-deftest test-darwin-notify-telegram-skips-no-token ()
  "agent--notify-telegram should skip when bot token is empty.
Verifies call-process is NOT called and the skip message is logged."
  (let ((agent-telegram-bot-token "")
        (agent-telegram-chat-id "123456")
        (call-count 0)
        (logged-messages nil))
    (cl-letf (((symbol-function 'call-process)
      ...
      (agent--notify-telegram "test message"))
...
(ert-deftest test-darwin-notify-telegram-skips-no-chat-id ()
  "agent--notify-telegram should skip when chat-id is empty.
Verifies call-process is NOT called."
  (let ((agent-telegram-bot-token "some-token")
        (agent-telegram-chat-id "")
        (call-count 0))
    (cl-letf (((symbol-function 'call-process)
               (lambda (&rest _) (cl-incf call-count) 0)))
      (agent--notify-telegram "test message"))
...
(ert-deftest test-darwin-notify-telegram-skips-both-empty ()
  "agent--notify-telegram should skip when both token and chat-id are empty.
Verifies call-process is NOT called."
  (let ((agent-telegram-bot-token "")
        (agent-telegram-chat-id "")
        (call-count 0))
...
(ert-deftest test-darwin-notify-telegram-sends-correct-payload ()
  "agent--notify-telegram should construct correct JSON payload and call curl.
Mocks call-process to capture the arguments and return a success response."
  (let ((agent-telegram-bot-token "test-token-123")
        (agent-telegram-chat-id "987654321")
        (captured-args nil)
    ...
    (cl-letf (((symbol-function 'call-process)
               (lambda (program &optional infile destination display &rest args)
                 ...
                 0)))
      (agent--notify-telegram "*Test Cycle Complete*\nElapsed: 10s"))
...
(ert-deftest test-darwin-notify-telegram-detects-success ()
  "agent--notify-telegram should detect success from \"ok\":true in response.
...
(DESTINATION=t in production code) and verifies the success message is logged."
  (let ((agent-telegram-bot-token "test-token")
        (agent-telegram-chat-id "123")
        (logged-messages nil))
    (cl-letf (((symbol-function 'call-process)
...
(ert-deftest test-darwin-notify-telegram-detects-failure ()
  "agent--notify-telegram should detect failure from non-ok response.
...
(ert-deftest test-darwin-notify-telegram-handles-empty-response ()
  "agent--notify-telegram should handle empty curl response without error.
...
(ert-deftest test-darwin-notify-telegram-escapes-special-chars ()
  "agent--notify-telegram should handle special characters in message via json-serialize.
...
;;; --- agent--notify-on-exit tests ---

(ert-deftest test-darwin-notify-on-exit-with-message ()
  "agent--notify-on-exit should call agent--notify-telegram when message is set."
  (let ((agent-cycle-result-message "*Darwin Cycle Complete*\nTool calls: 5")
        (notify-called nil)
        (captured-message nil))
    (cl-letf (((symbol-function 'agent--notify-telegram)
               (lambda (msg)
                 (setq notify-called t)
                 (setq captured-message msg))))
      (agent--notify-on-exit))
...
(ert-deftest test-darwin-notify-on-exit-without-message ()
  "agent--notify-on-exit should do nothing when result-message is nil."
  (let ((agent-cycle-result-message nil)
        (notify-called nil))
    (cl-letf (((symbol-function 'agent--notify-telegram)
               (lambda (_msg)
                 (setq notify-called t))))
...
(ert-deftest test-darwin-notify-on-exit-empty-string-no-send ()
  "agent--notify-on-exit should NOT send when message is empty string.
...
stringp and non-empty before sending."
  (let ((agent-cycle-result-message "")
        (notify-called nil))
    (cl-letf (((symbol-function 'agent--notify-telegram)
               (lambda (_msg)
                 (setq notify-called t))))
...
(ert-deftest test-darwin-notify-on-exit-non-string-no-send ()
  "agent--notify-on-exit should NOT send when message is not a string.
If agent-cycle-result-message is accidentally set to a non-string value
(e.g., t, a number, a symbol), the function should not attempt to send.
Tests integer 42 -- stringp returns nil for all non-strings."
  (let ((agent-cycle-result-message 42)
        (notify-called nil))
    (cl-letf (((symbol-function 'agent--notify-telegram)
               (lambda (_msg)
                 (setq notify-called t))))
...
(ert-deftest test-darwin-notify-on-exit-boolean-true-no-send ()
  "agent--notify-on-exit should NOT send when message is t.
...
agent--notify-telegram, which would fail inside json-serialize."
  (let ((agent-cycle-result-message t)
        (notify-called nil))
    (cl-letf (((symbol-function 'agent--notify-telegram)
               (lambda (_msg)
                 (setq notify-called t))))
...
(ert-deftest test-darwin-notify-on-exit-registered-in-hook ()
  "agent--notify-on-exit should be registered in kill-emacs-hook."
  (should (memq 'agent--notify-on-exit
                (default-value 'kill-emacs-hook))))

;;; --- agent--cycle-complete-p expanded phrase tests ---

(ert-deftest test-darwin-cycle-complete-cycle-is-done-phrase ()
  "agent--cycle-complete-p should match 'cycle is done' phrase.
...
(ert-deftest test-darwin-cycle-complete-cycle-summary-no-false-positive ()
  "agent--cycle-complete-p should NOT match 'cycle summary' without HISTORY.
...
;;; --- agent--cycle-complete-p expanded alternation tests ---
...
(ert-deftest test-darwin-cycle-complete-not-all-steps-done-no-false-positive ()
  "agent--cycle-complete-p should NOT match 'Not all steps done'.
...
(ert-deftest test-darwin-cycle-complete-sentinel ()
  "agent--cycle-complete-p should return t when CYCLE_COMPLETE sentinel is present.
...
(ert-deftest test-darwin-cycle-complete-sentinel-case-sensitive ()
  "agent--cycle-complete-p should NOT match lowercase cycle_complete.
...
(ert-deftest test-darwin-cycle-complete-sentinel-not-substring ()
  "agent--cycle-complete-p should NOT match CYCLE_COMPLETE as a substring.
...
(ert-deftest test-darwin-cycle-complete-sentinel-not-in-sentence ()
  "agent--cycle-complete-p should NOT match CYCLE_COMPLETE inside a sentence.
...
(ert-deftest test-darwin-cycle-complete-sentinel-at-buffer-start ()
  "agent--cycle-complete-p should match CYCLE_COMPLETE at the start of buffer.
...
(ert-deftest test-darwin-notify-telegram-detects-success-with-whitespace ()
  "agent--notify-telegram should detect success even with whitespace in JSON.
...
(ert-deftest test-darwin-notify-telegram-no-false-positive-on-error-with-ok-substring ()
  "agent--notify-telegram should NOT detect success when error message contains ok.
...
(ert-deftest test-darwin-notify-telegram-detects-success-boolean-true ()
  "agent--notify-telegram should detect success when :ok is JSON true (not string).
...
(ert-deftest test-darwin-notify-telegram-detects-failure-on-ok-false ()
  "agent--notify-telegram should detect failure when :ok is JSON false.
...
;;; --- agent--cycle-complete-p finished.*cycle tightening tests ---
...
(ert-deftest test-darwin-cycle-complete-finished-bicycle-no-false-positive ()
  "agent--cycle-complete-p should NOT match 'finished working on the bicycle'.
...
(ert-deftest test-darwin-cycle-complete-finished-a-cycle ()
  "agent--cycle-complete-p should match 'finished a cycle'.
...
(ert-deftest test-darwin-notify-telegram-handles-curl-error ()
  "agent--notify-telegram should catch curl errors and log FAILED message.
...
;;; --- agent--notify-telegram JSON parse error test ---

(ert-deftest test-darwin-notify-telegram-logs-json-parse-error ()
  "agent--notify-telegram should log JSON parse errors with detail.
...
;;; --- agent--cycle-complete-p word-boundary test ---

(ert-deftest test-darwin-cycle-complete-finished-cycles-no-false-positive ()
  "agent--cycle-complete-p should NOT match 'finished the cycles'.
...
;;; --- agent--cycle-complete-p narrowing tests ---

(ert-deftest test-darwin-cycle-complete-widens-narrowed-buffer ()
  "agent--cycle-complete-p should widen before searching.
...
bare widen would permanently remove the narrowing as a side effect."
  (with-temp-buffer
    ...
    (let ((narrow-start (point-min))
      ...
      (agent--cycle-complete-p (current-buffer))
...
(ert-deftest test-darwin-cycle-complete-sentinel-widens-narrowed-buffer ()
  "agent--cycle-complete-p should detect CYCLE_COMPLETE sentinel even when narrowed.
...
;; the :safe predicate.  The guard logic in agent-run-cycle uses the
...
(ert-deftest test-darwin-cycle-timeout-safe-predicate ()
  "agent-cycle-timeout :safe predicate should reject nil/0/-1, accept positive integers.
Tests the actual registered :safe predicate via safe-local-variable-p,
not a local lambda copy.  The guard in agent-run-cycle uses the same
pattern and falls back to 7200 (the defcustom default) when the guard fails."
  ...
  (should-not (safe-local-variable-p 'agent-cycle-timeout nil))
  (should-not (safe-local-variable-p 'agent-cycle-timeout 0))
  (should-not (safe-local-variable-p 'agent-cycle-timeout -1))
  (should-not (safe-local-variable-p 'agent-cycle-timeout "foo"))
  ;; Verify the actual :safe predicate accepts valid values
  (should (safe-local-variable-p 'agent-cycle-timeout 7200))
  (should (safe-local-variable-p 'agent-cycle-timeout 3600))
  ;; Verify the defcustom default matches the guard fallback value
  (should (eq (default-value 'agent-cycle-timeout) 7200)))

(ert-deftest test-darwin-cycle-max-turns-safe-predicate ()
  "agent-cycle-max-turns :safe predicate should reject nil/0/-1, accept positive integers.
Tests the actual registered :safe predicate via safe-local-variable-p.
The guard in the continuation hook uses the same pattern and falls back
to 40 (the defcustom default) when the guard fails."
  ;; Verify the actual :safe predicate rejects bad values
  (should-not (safe-local-variable-p 'agent-cycle-max-turns nil))
  (should-not (safe-local-variable-p 'agent-cycle-max-turns 0))
  (should-not (safe-local-variable-p 'agent-cycle-max-turns -1))
  (should-not (safe-local-variable-p 'agent-cycle-max-turns "foo"))
  ;; Verify the actual :safe predicate accepts valid values
  (should (safe-local-variable-p 'agent-cycle-max-turns 40))
  (should (safe-local-variable-p 'agent-cycle-max-turns 100))
  ;; Verify the defcustom default matches the guard fallback value
  (should (eq (default-value 'agent-cycle-max-turns) 40)))

;;; --- Telegram credential :safe removal tests ---
;; These tests verify that agent-telegram-bot-token and
;; agent-telegram-chat-id intentionally lack :safe predicates.
...
(ert-deftest test-darwin-telegram-bot-token-no-safe-predicate ()
  "agent-telegram-bot-token should NOT have a :safe predicate.
...
session file from silently redirecting notifications to an attacker's bot."
  ...
  (should-not (safe-local-variable-p 'agent-telegram-bot-token "some-token"))
  (should-not (safe-local-variable-p 'agent-telegram-bot-token ""))
  (should-not (safe-local-variable-p 'agent-telegram-bot-token 12345)))

(ert-deftest test-darwin-telegram-chat-id-no-safe-predicate ()
  "agent-telegram-chat-id should NOT have a :safe predicate.
...
preventing a tampered session file from silently redirecting notifications."
  (should-not (safe-local-variable-p 'agent-telegram-chat-id "123456"))
  (should-not (safe-local-variable-p 'agent-telegram-chat-id ""))
  (should-not (safe-local-variable-p 'agent-telegram-chat-id 123456)))
