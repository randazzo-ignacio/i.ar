...
reconfiguration."
  (format (iar--load-prompt "loop_hard_stop")
...
- (:stop t :stop-reason reason) if hard threshold reached (request stopped)"
  (let* ((name (plist-get info :name))
    ...
    (let* ((total (1+ repeat-count))
          ...
          (effective-soft
           (let ((s iar-loop-soft-threshold))
             (if (and (integerp s) (> s 0)) s 3)))
          (effective-hard
           (let ((h iar-loop-hard-threshold))
...
(defun my-gptel--loop-guard-setup ()
  ...
  (add-hook 'gptel-pre-tool-call-functions #'iar--mygptel--loop-guard))
